...
\begin{definition}
    A network $N'$ is an \emph{object reduction} of $N$ if
    \begin{enumerate}
        \item $S' = S$.
        \item There is a bijection $f: C_N \to C_{N'}$ such that $(s, c) \in R$
              iff $(s, f(c)) \in R'$.
...
\begin{proof}[Proof (sketch)]
    ...
    Taking any network $N$ and claims $c, d$, we first remove all reports for
    other claims to produce $N'$; this preserves rankings by
    \classicalindependence{}. Taking $N''$ to be any object reduction of $N'$,
    we ensure the claims associated with $c$ and $d$ are for different objects,
    and rankings are preserved by \objectirrelevance{}. As before, we show
    $|\src_N(c)| \le |\src_N(d)|$ implies $c \cle_N^T d$ and $|\src_N(c)| <
    |\src_N(d)|$ implies $c \clt_N^T d$ by induction on $|\src_N(d)| -
    |\src_N(c)|$. In the base case $|\src_N(c)| = |\src_N(d)|$, the fact that
    the claims in $N''$ associated with $c$ and $d$ are for different objects
    crucially ensures we can choose an automorphism on $N''$ which swaps $c$
    and $d$ (and we swap $\src_N(c) \setminus \src_N(d)$ with $\src_N(d)
    \setminus \src_N(c)$). \symmetry{} then gives $c \ceq_N^T d$. For the
    inductive step we apply \freshposresp{} as in the proof of
    \cref{td_new_prop_symm_fpr_single_object_voting}; the fact that we removed
    all reports for claims other than $c$ and $d$ in constructing $N'$ ensures
    the relevant new source is fresh. Finally, this shows $c \cle_N^T d$ iff
    $|\src_N(c)| \le |\src_N(d)|$.
\end{proof}
