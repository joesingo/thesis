...
Formally, the input to the truth discovery problem is a \emph{network}, defined
as follows.
...
\begin{definition}
    ...
    \begin{itemize}
        \item $S$ is a finite set of \emph{sources}.
        \item $O$ is a finite set of \emph{objects}.
        \item $D = \{D_o\}_{o \in O}$ are the \emph{domains} of the objects,
              where each $D_o$ is a finite set of values. We write
...
The set $R$ is the core data associated with the network: we interpret $(s, o, v)
...
axioms. Such axioms require that some aspect of the output is independent of
irrelevant parts of the input. The original example is Arrow's
\emph{Independence of Irrelevant Alternatives} in voting
theory~\cite{arrow1952}, which says the ranking of candidates $A$ and $B$
should depend only on the individual rankings of $A$ and $B$, not on any
``irrelevant" alternative $C$. Closer to our setting, independence in judgement
aggregation says that the collective acceptance of a report $\phi$ does not
depend on how the individuals accept some other report
$\psi$~\cite{endriss2016ja}.

Adapted to the truth discovery setting, a classical independence axiom would
require the ranking of claims $c$ and $d$ to depend only on the sources
reporting $c$ and $d$; not on the sources for other claims, for instance. Such
an axiom is clearly \emph{undesirable} for truth discovery. Indeed, consider
again the network $N$ from \cref{td_new_fig_intro_example}. As we have argued
informally, claim $c$ is intuitively weaker than $d$ precisely because of how
their sources interact with other claims in the network. Nevertheless, we state
this axiom as a point of comparison with classical social choice problems such
as voting.
...
If \classicalindependence{} is too strong, what is an appropriate independence
notion? Note that in \cref{td_new_fig_intro_example}, all claims are
``reachable" from each other via common sources or objects. In this sense, all
parts of the network play a role in the ranking of any pair of sources or
claims. In contrast, the network in \todo{fig} consists of two disjoint
sub-networks which do not interact. Intuitively, the top part of the network
should not affect the rankings in the bottom and vice versa. We formalise this
idea via unions of disjoint networks.\footnotemark{}
...
also.
%
The following independence axiom says that the ranking of sources and claims is
unaffected by the addition of a disjoint network.
...
% \disjointindependence{} also has a graph-theoretic interpretation. To any
% network $N$ we can associate a graph over nodes $S \cup C \cup O$ with edges
% from sources to their claims and from claims to their associated objects. The
% \emph{connected components} of this graph correspond to disjoint networks, and
% $N$ is the union of these networks.
