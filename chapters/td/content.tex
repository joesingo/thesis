...
The set of all networks is denoted by $\N$.
%
...
write $\obj(c) = o$ in this case, and let $C$ denote the set of all claims.
With slight abuse of notation, we write $(s, c)$ for the report $(s, o, v)$.
Then $N$ can also be viewed as a subset of $S \times C$, i.e. a relation
between sources and claims. In fact, we will take this claim-centric view in
the remainder of the paper, with objects and values only playing a role insofar
as they tell us which claims are in conflict with one another.
...
$S, O$, $D$ and $C$ left implicit.
%
The following notation is convenient to
extract information about various components of a network. For $c \in C$ and
$s \in S$, write
\begin{align*}
...
\to \R$ for only a subset $\N(T) \subseteq \N$ of networks.
...
\begin{figure}
    \centering
    \includegraphics[scale=0.25]{coherence_intro}
    \caption{
