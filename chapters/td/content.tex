...
% Let $(\D, T_0, U)$ be as defined for Sums in \cref{td_new_def_sums}. Sums is
% clearly bounded by definition of $\D$.



% It will
% prove more convenient to express $U$ as a composition $U_2 \circ U_1$, where
% $U_1$ performs the sum step, and $U_1$ normalises scores by dividing by the
% maximum. In what follows, for a set of numbers $X \subseteq \R$, write
% \[
%     \norm{X} = \begin{cases}
%         \frac{1}{\max_{x \in X}{|x|}},& \max_{x \in X}{|x|} \ne 0 \\
%         0,& \text{ otherwise}.
%     \end{cases}
% \]

% \begin{lemma}
%     Define $U_1: \D \to \D$ by $U_1(T) = T'$, where
%     \begin{align*}
%         T'_N(s) &=
%             \sum_{c \in \claims_N(s)}{
%                 T_N(c)
...
%             }
%     \end{align*}
%     and $U_2: \D \to \D$ by $U_2(T) = T'$, where
%     \begin{align*}
%         T'_N(s) &=
%             \norm{T_N(S)} \cdot T_N(s)
%         \\
%         T'_N(c) &=
%             \norm{T_N(C)} \cdot T_N(c).
%     \end{align*}
%     Then $U = U_2 \circ U_1$.
% \end{lemma}
...
\todo{Mention Voting axioms. Proofs are similar to Weighted Agreement.}
...
simplicity we assume $\sums$ converges on all input networks.\footnote{While
\textcite{pasternack2010} do not consider convergence issues, Sums is an
adaptation of the \emph{Hubs and Authorities} algorithm for ranking web pages,
for which \textcite{kleinberg1999} proves convergence.} We also write $T^n =
U^n(T^0)$ for the $n$-th step of the iteration of Sums.
