...
language with two new notions: that of a source having \emph{expertise} on a
formula, and a formula being \emph{sound} for a source to report. Consequently
...
\begin{chapteroutline}
    In \cref{tt_sec_preliminaries} we outline the logical framework for reasoning
...
\cref{tt_ex_informal_example} are the patients $\patientA{}$ and $\patientB{}$. We
...
is indistinguishable from some $\phi$ state. Thus, $\S_i\phi$ means
...
$L(\sigma) \subseteq \worlds$, called the \emph{conjecture} of $L$
on $\sigma$.\footnotemark{} We say $L$ \emph{implies} $S
...
\cref{tt_lemma_spec_active_cell}, we can interpret $W \specleq W'$ as saying that
...
\begin{figure}
    ...
    \caption{
        World $W$ from \cref{tt_ex_qstar_partitions}. Note that for brevity we do
...
$\sigma$ are conjectured to be sound.
...
\begin{theorem}
    ...
    \begin{equation}
        \label{tt_eq_conditioning_tt_characterisation}
        W \speclt W'
            \implies
...
Like \credulity{}, \cref{tt_eq_conditioning_tt_characterisation} is a principle of
...
\begin{proof}[Proof of \cref{tt_thm_conditioning_tt_characterisation}]
    ...
    \cref{tt_eq_conditioning_tt_characterisation} holds if and only if $L(\sigma)
    ...
    \cref{tt_eq_conditioning_tt_characterisation}, there is $W'' \speceq W$ such
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item The variable-based conditioning method $L_\vbc{}$ is \emph{not}
              ...
              such that $W'' < W'$. Hence
              \cref{tt_eq_conditioning_tt_characterisation} fails, and $L_\vbc{}$
