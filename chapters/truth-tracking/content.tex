...
\todo{Final property was called ``refinement at $c$'' in
\cref{chapter_belief_change}. Note this.}
...
\cref{tt_eq_conditioning_tt_characterisation} is also intuitively similar to
...
\begin{definition}
    ...
    \N_{0}$, the \emph{score-based method} $L_d$ is given by
    \[
        L_d(\sigma)
        = \argmin_{W \in \Xsnd_\sigma}{
            \sum_{\tuple{i, c, \phi}}{
                d(W, \tuple{i, c, \phi})
            }
        }.
...
in case $c$. The conjecture $L_d(\sigma)$ consists of the worlds satisfying the
soundness constraints of $\sigma$ with minimal disagreement score, computed as
the sum of the disagreement on each report.
%
...
\todo{truth-tracking characterisation for score-based operators, and discussion
for excess-min specifically.}
