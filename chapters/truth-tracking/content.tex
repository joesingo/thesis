...
as in \cref{chapter_belief_change} but with minor differences. Most
importantly, we exclude the special source $\ast$.
...
\begin{figure}
    ...
    \caption[
        An example world formalising \cref{tt_ex_informal_example}.
    ]{
...
sequence of reports. Such sequences are inputs to \emph{learning
methods}.\footnotemark{}

\footnotetext{
    We use the terminology ``method'' instead of the usual ``operator'' for
    consistency with the learning theory literature.
}

\begin{definition}
    A \emph{learning method} $L$ maps each input sequence $\sigma$ to a set of
    worlds $L(\sigma) \subseteq \worlds$, called the \emph{conjecture} of $L$
    on $\sigma$.
\end{definition}
...
This definition is the main point of difference between the framework of the
present chapter and of \cref{chapter_belief_change}. Firstly, we no longer
consider separate belief and knowledge outputs for each sequence $\sigma$; the
conjecture $L(\sigma)$ should be thought of as analogous to the belief set of an
operator in the sense of \cref{kr_def_operator}.
%
Secondly, we take a purely semantic view here by considering the output of a
method to be a set of \emph{worlds} instead of a collection of \emph{formulas}.
Of course, an operator in the sense of \cref{chapter_belief_change} defines a
method by setting $L(\sigma) = \operatorname{mod}(B^\sigma)$, and a method
defines the belief set for an operator in the manner of model-based operators
(\cref{kr_def_model_based}). The difference is therefore in presentation only,
and the semantic viewpoint will prove to be more convenient in what follows.
...
\begin{figure}
    ...
    \caption[
        World $W$ from \cref{tt_ex_qstar_partitions}.
    ]{
...
Indeed, take $c \in \css$. Letting $I_W$ denote the set of sources with
...
Next, we reconsidered the belief and expertise operators of
...
truth-tracking can be made more informative, in the sense of revealing more
