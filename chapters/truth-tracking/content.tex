...
on sources. We presently sketch a somewhat more extreme solution, where such
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item Requiring a strict majority of experts as above may be deemed too
              ...
              Now let $S_\plurality$ be the set of worlds in which there is at
              least a plurality of correct sources, i.e. $W \in S_\plurality$
              iff $|J_W| > |I|$ for all coalitions $I \ne J_W$. Note that $J_W$
              need not be a majority: we may have $|J_W| \le
              \frac{|\srcs|}{2}$.

              Then for any $W \in S_\plurality$, $i \in J_W$, $c \in \css$ and
              $\phi \in \bel{W}{i}{c}$, we have $Q^*_\b[W] \cap S_\plurality, c
              \models \phi$. That is, any $\b$-truth-tracking method -- when
              restricted to consider only worlds in $S_\plurality$ --
              eventually learns the correct beliefs of all sources in $J_W$.
              \todo{Prove this, and give an example where $J_W$ are case-wise
              in the minority.}
