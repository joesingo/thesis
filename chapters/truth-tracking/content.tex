...
informativeness of truth-tracking via belief-based streams. In order to improve
this bound so that more can be learned, it is necessary to exclude certain
problematic worlds. For instance, consider the simple case $\propvars = \{p\}$.
One can construct $W$ and $W'$ such that all sources believe $p$ in all cases
-- so that $W \speceq_\b W'$ -- and where $p$ holds in $W$ but $\neg{p}$ holds
in $W'$. Consequently, $Q^*_\b[W]$ does not even determine $p$, despite the
fact that all sources are unanimously correct in their beliefs in $W$.

The problem here is the existence of ``degenerate'' worlds such as $W'$. In the
first part of this chapter, non-experts such as the sources in $W'$ were
obliged to report both $p$ and $\neg{p}$, thus signalling their lack of
expertise; this is no longer the case when considering belief-based streams.

In probability-based Jury theorems~\cite{grofman1983thirteen} such degenerate
worlds occur with low probability due to the choice of the probabilistic model
and assumption placed on sources. One possible solution in our framework is to
exclude such worlds from consideration entirely. For example, one could look at
subsets $S \subseteq \worlds$ and consider the extent to which $Q^*_\b[W] \cap
S$ reveals information about $W \in S$. In other words, what can $\b$-streams
tell us about $W$ if we do not need to worry about worlds in $\worlds \setminus
S$? We leave these considerations for future work.

% We sketch a somewhat more extreme solution to the problem, where such worlds
...
%to which $Q^*_\b[W] \cap S$ reveals information about $W$. In place of a
%systemic study, we explore this concept via examples of particular choices of
%$S$.

%\begin{example}
%    Fix a belief structure $\b$.
%    \begin{enumerate}
%        \item For a world $W$, say a set of sources $I \subseteq \srcs$ is a
%              \emph{$W$-coalition} if for each $c \in \css$ the set $\bigcup_{i
%              \in I}\{\phi \mid W, c \models_\b \B_i\phi\}$ is consistent. Let
%              $\mathcal{I}_W$ denote the set of $W$-coalitions.
%              %
%              Then take $S^\plurality_\b$ to be the set of worlds $W$ in which
%              the set of sources with correct beliefs in all cases forms a
%              plurality, i.e.
%              \[
%                  W \in S^\plurality_\b
%                  \iff
%                    \exists I \in \mathcal{I}_W \text{ s.t. }
%                        (i \in I, W, c \models_\b \B_i\phi \implies W, c
%                        \models \phi)
%                        \text{ and }
%                        (J \in \mathcal{I}_W \setminus \{I\} \implies |I| > |J|)
%                  \}
%              \]
%    \end{enumerate}
%\end{example}
