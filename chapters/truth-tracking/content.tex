...
\todo{Example of two worlds which are equivalent and two which are not.}
...
whenever $W \in T_\sigma$. \todo{Richard says an example is needed here.}
%
...
\repetition{}).\footnotemark{} \todo{Show.}
...
weak model-based operator from \cref{kr_def_weakop} -- fails \credulity{}.
\todo{Show} Some amount of or \emph{non-monotonic} reasoning, as captured by
...
\begin{example}
    ...
    then observes both patients. \todo{Richard wants the figure repeated here.}
    Since the actual valuations $v^W_{\patientA}$
