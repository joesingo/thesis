...
\begin{notes}
    \begin{itemize}
        ...
        \item A \emph{belief stream} then enumerates the beliefs of each
              ...
              before, and the truth-tracking problem therefore becomes harder;
              this will be formalised below.  \todo{check this.}
    \end{itemize}
\end{notes}
...
case are consistent. Also note the similarities with conditioning methods in
\cref{tt_sec_conditioning_methods}, where a plausibility order is used to
...
with $\B_i\phi$ replacing $\S_i\phi$. Likewise, we have a notion of
\emph{$\b$-solvability}.
...
Again mirroring the definitions above, a preorder $\specleq_\b$ can be defined
on worlds, where
\[
    W \specleq_\b W'
    \iff
...
\begin{proof}[Proof (sketch)]
    ...
    ``only if'': This can be shown by an argument idential to that of
...
\todo{Address gap: is there a single method $L$ which solves $Q^*_\b$ for all
$\b$?}
