...
%
Since the \axiomref{Success} postulate ($\alpha \in K \ast \alpha$)
...
%one to model unreliable sources by dropping the \axiomref{Success}
...
\paragraph{Contributions.} Our contributions are threefold. First, we develop a
...
\begin{chapteroutline}
    In \cref{kr_sec_framework} we develop the formal framework.
...
$\lor$, $\limplies$, $\liff$ and $\falsum$ as
