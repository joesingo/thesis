...
\begin{definition}[\textcite{konieczny2002merging,Konieczny_2011}]
    ...
    \[
        ...
        =
        \argmin\limits_{v \in \propmods{\mu}}{
            \sum_{i=1}^{n}{
...
constraint $\mu$ minimising the sum of distances to each formula $\phi_i$, with
the distance between $v$ and $\phi_i$ interpreted as the minimal distance
between $v$ and some $\phi_i$ model.
...
\begin{restatable}{theorem}{restatekrmergingthm}
    ...
    For any $\mu$ and $\Psi$,
    \begin{enumerate}
        \item $\vbcmergingop_\mu(\Psi) \equiv \Delta^{d_H, \Sigma}_\mu(\Psi)$
        \item $\pbcmergingop_\mu(\Psi) \equiv \Delta^{d_D, \Sigma}_\mu(\Psi)$
        \item $\exmmergingop_\mu(\Psi) \equiv \Delta^{d_D, \Sigma}_\mu(\Psi)$
    \end{enumerate}
\end{restatable}

The proof can be found in \cref{kr_app_sec_thm_merging_operators}. It follows
that $\vbcmergingop$, $\pbcmergingop$ and $\exmmergingop$ each satisfy the
