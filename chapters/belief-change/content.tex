...
\paragraph{Chapter Outline.} In \cref{sec:framework} we develop the formal
...
(\labelcref{item:replacement_equivalents_e_s}) states syntax-irrelevance
properties.
%
...
%
(\labelcref{item:sound_neg_pair}) says that if both $\phi$ and $\neg\phi$ are
true up to the expertise of $i$, then $i$ cannot have expertise on $\phi$.
%
Finally, (\labelcref{item:star_exp}) says that the reliable source $\ast$ has
...
\begin{itemize}
    ...
    \item[\labelcref{item:e_symmetric}.]
        ...
        $\E_i\phi$ by (\labelcref{item:replacement_equivalents_e_s}) we get
        $\E_i\phi \leftrightarrow \E_i\neg\phi$.
    ...
    \item[\labelcref{item:exp_on_all_variables}.]
        ...
        assumption and the properties in (\labelcref{item:e_symmetric}),
    ...
    \item[\labelcref{item:sound_neg_pair}.]
        ...
        (\labelcref{item:e_and_s_implies_phi}) gives $W, c \models \phi$. On
        the other hand, $W, c \models \E_i\phi$ gives $W, c \models
        \E_i\neg\phi$ by (\labelcref{item:e_symmetric}), so $W, c \models
        \E_i\neg\phi \land \S_i\neg\phi$; by
        (\labelcref{item:e_and_s_implies_phi}) again we have $W, c \models
...
\begin{proof}
    ...
    \begin{inlinelist}
        ...
        \item \label{item:partition_equiv} for each $i \in \Srcs$ and $U \subseteq
    ...
    the statement of the \lcnamecref{prop:elementary_characterisation}. Take $c
...
\begin{proof}\leavevmode
    \begin{enumerate}
        ...
        \item Suppose $\phi \in K^\sigma_c$ and $\neg\psi \in \cnprop(\phi)$.
              ...
              (\labelcref{item:e_and_s_implies_phi}) that $\S_i\psi \land \neg
          ...
          \item Suppose $\tuple{i, c, \phi} \in \sigma$ and $\E_i\phi \in
                ...
                (\labelcref{item:e_and_s_implies_phi}) again we have $\E_i\phi
...
\begin{example}
    ...
    Set $\Srcs, \C$ and $\propvars$ as in \cref{ex:hospital_world}, and consider
    the sequence
    \[
        \sigma
        = (
...
$\le_0$-minimal in $\X_\sigma$. In fact, the converse is also true.
    ...
    \begin{proof}
        ...
        \footnotetext{
            ...
            \cref{claim:w_j_in_rho}. Otherwise, $W_{j+1} \in \Y_{\rho_{j+1}}
...
\end{inlinelist}\footnote{
    ...
    \begin{proof}
        ...
        \not< W$. By property \labelcref{item:strict_ineq_preserved} of $\le$,
...
\begin{proof}
    ...
    part (\labelcref{item:e_and_s_implies_phi}), we get $W', c \models \phi$.
...
\begin{proof}
    ...
    operator.
        ...
        \begin{proof}
            ...
            \simeq W_2$. By property (\labelcref{item:anonymity}), $W_1 \simeq
            \pi(W_1)$. By transitivity of ${\simeq}$ we get $W_1 \simeq W_2$ as
            desired.
        \end{proof}

    Now, from the basic postulates, property
    (\labelcref{item:conditioning_impossibility_first}) and
...
\begin{proof}[Proof of \cref{prop:conditioning_selectivity_characterisation}]
...
\begin{proof}
    ...
    in \labelcref{eqn:hbnd_inclusion}, and we are done.
    ...
    \labelcref{eqn:hbnd_application} we get $v \in M_c$, and the proof is
    complete.
...
\cref{ex:hospital_ex_formalised}. According to that view we should have
$\E_i{p}
