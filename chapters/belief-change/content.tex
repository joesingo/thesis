...
\begin{proof}
    ...
    \todo{Proposition 23 from online notes}
\end{proof}
...
Note that \cref{kr_lemma_ec_selective_propmodels} immediately implies
...
\cref{kr_lemma_ec_selective_propmodels} also shows that propositional beliefs
...
under additional mild assumptions. In what follows, recall that $G^\snd_\sigma$
denotes the collection with $(G^\snd_\sigma)_c = \{\S_i\phi \mid \tuple{i,
\phi} \in \sigma \rs c\}$.

\begin{theorem}
    \label{kr_thm_ec_selectivity_characterisation}
    A model-based operator satisying \consistency{} and $K^\sigma =
    \cn(G^\snd_\sigma)$ for all $\sigma$ is EC-selective if and only
    if it satisfies \determination{}.
\end{theorem}

\todo{Write up from online notes.}

In particular, \cref{kr_thm_ec_selectivity_characterisation} applies to all
operators satisfying the basic postulates and $K^\emptyset = \cn(\emptyset)$,
by \cref{kr_prop_prior_knowledge}.

\todo{Consider weak EC-selectivity, where we only require $\neg\E_i{f_\sigma(i,
c, \phi)} \notin B^\sigma_c$.}
