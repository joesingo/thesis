...
$\phi_i$.
%
In this way, any operator gives rise to a merging operator -- defined up to
$\lprop$ equivalence -- with
\begin{equation}
...
\emph{IC merging operators}. In each postulate, all formulas $\phi \in \Psi$
...
\begin{axiom}[\icpostulate{4}]
    If $\phi \propentails \mu$ and $\phi' \propentails \mu$, then if
    $\Delta_\mu(\{\phi, \phi'\}) \land \phi$ is consistent, $\Delta_\mu(\{\phi,
    \phi'\}) \land \phi'$ is also consistent
\end{axiom}
