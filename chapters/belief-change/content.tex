...
linked. As set out in \cref{chapter_td} in the context of truth discovery, we
should accept reports from sources we believe are trustworthy, and we should
trust sources whose reports turn out to be reliable. Trust and belief should
also be revised in tandem, so that we may increase or decrease trust in a
source as more reports are received, and revoke or reinstate previous reports
...
framework in \cref{chapter_expertise}. Firstly, we consider only a finite
...
This chapter is an extension of \textcite{singleton_booth_22}. New material
...
there is a \emph{completely reliable} source in $\srcs$, which in a nod to the
AGM tradition we denote by
...
describe the ``ontic'' facts of the world in each case $c \in \C$. We use lower case
...
\begin{figure}
    \centering
    \input{chapters/belief-change/tikz/world_example.tex}
    \caption{
...
\]
That is, $\Phi$ is a $c$-consequence of a collection $G$ if $\Phi$ holds in
case $c$ for every model $W$ of $G$.
%
We write $\cn(G)$ for the collection $\{\cn_c(G)\}_{c \in \C}$.
...
on. For instance, when $G$ is closed any expertise formula $\E_i\phi$ is either
present in $\Gamma_c$ for all cases $c$ or for none, reflecting the fact that
expertise is fixed across cases. We also have that propositional beliefs about
the world in each case $c$ are closed under classical consequences.

\begin{proposition}
    Suppose a collection $G = \{\Gamma_c\}_{c \in \C}$ is closed. Then
    \begin{enumerate}
        \item $\E_i\phi \in \Gamma_c$ if and only if $\E_i\phi \in \Gamma_d$.
        \item $\cnprop\proppart{\Gamma_c} = \proppart{\Gamma_c}$.
    \end{enumerate}
\end{proposition}

\begin{proof}\leavevmode
    \begin{enumerate}
        \item Suppose $\E_i\phi \in \Gamma_c$. Since $G = \cn(G)$ we have
              $\Gamma_d = \cn_d(G)$. Take $W \in \mods(G)$. Then $W$ satisfies
              all formulas of $\Gamma_c$ in case $c$, so $W, c \models
              \E_i\phi$.  As expertise is independent of case, this means $W, d
              \models \E_i\phi$. By definition of $d$-consequences, we have
              $\E_i\phi \in \cn_d(G) = \Gamma_d$ as required. The reverse
              direction holds by symmetry.
         \item Clearly $\proppart{\Gamma_c} \subseteq
               \cnprop\proppart{\Gamma_c}$. For the reverse inclusion, suppose
               $\phi \in \cnprop\proppart{\Gamma_c}$. We will show $\phi \in
               \cn_c(G)$, which implies $\phi \in \proppart{\Gamma_c}$ since
               $G$ is closed. Indeed, take $W \in \mods(G)$. Then, $W, c
               \models \psi$ for all $\psi \in \proppart\Gamma_c$. By
               definition of the semantics for propositional formulas, this
               means the valuation $v^W_c$ satisfies each $\psi$, so $v^W_c$ is
               a model of $\proppart{\Gamma_c}$. By the definition of the
               classical consequence operator $\cnprop$, this means $v^W_c$ is
               also a model of $\phi$, i.e. $W, c \models \phi$. Hence $\phi
               \in \cn_c(G)$ as required.
    \end{enumerate}
\end{proof}
...
With the framework set out, we can formally define the problem.  As input, we
consider sequences of \emph{reports}, typically denoted $\sigma$, where each
report is a triple $\tuple{i, c, \phi} \in \srcs \times \C \times \lprop$ with
$\phi \not\equiv \falsum$. Such a report represents that {\em source $i$
reports $\phi$ to hold in case $c$}. Note that we only allow sources to make
\emph{propositional} reports. A \emph{belief and expertise operator} produces a
collection of belief and knowledge sets on the basis of such sequences.

\begin{definition}
    \label{kr_def_operator}
    A \emph{belief and expertise operator} maps each sequence of reports
    $\sigma$ to a pair $\tuple{B^\sigma, K^\sigma}$, where $B^\sigma =
    \{B^\sigma_c\}_{c \in \C}$ is a collection of \emph{belief sets}
    $B^\sigma_c \subseteq \lext$ and $K^\sigma = \{K^\sigma_c\}_{c \in \C}$ is
    a collection of \emph{knowledge sets} $K^\sigma_c \subseteq \lext$.
\end{definition}
...
\cref{kr_item_trust_belief_link} expresses how trust affects belief: we should
believe reports from trusted sources.  It can also be seen as a form of
\emph{success} for ordinary sources, and implies AGM success when $i = \ast$
(by \cref{kr_prop_validities} part \cref{kr_item_star_exp} and \closure{}).
Note that these are the same ideas underlying the \emph{coherence} axioms for
truth discovery in \cref{chapter_td}. We illustrate the basic postulates by
formalising the introductory hospital example.
...
%
Syntactically, we have $K^\sigma = B^\sigma = \cn(G^\sigma_\snd)$.

\begin{proposition}
    \label{kr_prop_weakop_basic_postulates}
    \weakop{} satisfies all of the basic postulates.
\end{proposition}

\begin{proof}[Proof (sketch)]
    Most of the postulates are straightforward. Clearly \closure{} and
    \containment{} hold by \cref{kr_thm_model_based_characterisation}.
    \soundness{} holds by construction.  \kbound{} holds by monotonicity of
    $\cn$, since $K^\sigma = \cn(G^\sigma_\snd) \sqsubseteq \cn(G^\sigma_\snd
    \sqcup K^\emptyset)$. \priorext{} holds since $\X_\emptyset = \W \supseteq
    \X_\sigma$, so $K^\emptyset \sqsubseteq K^\sigma$. \rearr{} holds
    trivially, and \equivpost{} follows from the fact that $\S_i\phi$ is
    equivalent to $\S_i\psi$ whenever $\phi \equiv \psi$
    (\cref{kr_prop_validities} \cref{kr_item_replacement_equivalents_e_s}).

    In fact, the only non-trivial postulate is \consistency{}. To show this
    holds, take any $\ast$-consistent sequence $\sigma$. By definition, for
    each case $c$ the set of reports from $\ast$ is consistent, and is thus
    modelled by some valuation $v_c$. Forming a world $W$ with these valuations
    and with each ordinary source $i \ne \ast$ having the trivial one-cell
    partition $\Pi_i = \{\vals\}$, we have that \emph{all} $\phi \not\equiv
    \falsum$ are sound for each $i$ (since $\Pi_i[\phi] = \vals \ni v_c$), and
    all reports from $\ast$ are true, and in particular sound for $\ast$.
    Consequently $W \in \X_\sigma \subseteq \mods(K^\sigma)$, so $K^\sigma =
    B^\sigma$ are consistent.  \end{proof}

In fact, \weakop{} is the \emph{weakest} operator satisfying \closure{},
\containment{} and \soundness{}, in that for any other operator $\sigma \mapsto
\tuple{\widehat{B}^\sigma, \widehat{K}^\sigma}$ with these properties we have
$B^\sigma \sqsubseteq \widehat{B}^\sigma$ and $K^\sigma \sqsubseteq
\widehat{K}^\sigma$ for any $\sigma$.
%
...
plausibility total preorder on the knowledge $\X_\sigma$, and
...
basic postulates, using essentially the same argument as for \weakop{} in
\cref{kr_prop_weakop_basic_postulates}.
...
\condcons{} is another consistency postulate which says beliefs $B^\sigma$ can
only be inconsistent if the knowledge $K^\sigma$ is too. \duprem{} says that
the output does not change when a report $\tuple{i, c, \phi}$ in $\sigma$ is
repeated; together with \rearr{}, this means any duplicate reports may be
ignored.
...
Again, one can verify that \scorebasedop{} satisfies the basic postulates of
\cref{kr_sec_basic_postulates} by an argument similar to the one employed in
\cref{kr_prop_weakop_basic_postulates}. It can also be seen that $\X_\sigma$
and $\Y_\sigma$ are elementary, and \scorebasedop{} fails \incvac{}.
%
It follows from
...
Merging can be seen as a special case of our framework, if we impose an upper
