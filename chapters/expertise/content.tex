...
both singular and collective expertise are shown to be connected in a precise
...
\textcite{singleton2022expetise,singleton2021logic}, with new material
appearing in \cref{exp_sec_dynamic_extension}. Several of the main proofs have
been formalised with the Lean theorem
...
$\phi$, and $\S\phi$ as ``$\phi$ is sound for the source to
...
\begin{example}
    ...
    d$. The economist's report of $p \land \neg d$ is represented by the blue
    region. We see that while $M, ipd \not\models p \land \neg d$, all
    expertise sets containing the blue region also contain $ipd$, so $M, ipd
...
introspection axiom \textbf{5} not generally considered plausible for modelling
