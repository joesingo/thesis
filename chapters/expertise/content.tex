...
\Cref{sec_closure_properties} looks at how expertise may be closed under
...
multiple sources. \Cref{sec_dynamic_extension} introduces the dynamic extension
of the logic, and we conclude in \cref{sec_conclusion}. Several of the main
...
\begin{proof}
    ...
    Let $M = (X, P, V)$ be a model and $x \in X$. \refitem{item_truths_sound} and
    \refitem{item_e_global} are clear. For \refitem{item_weakening}, suppose $M, x
    ...
    \|\psi\|_M$, i.e. $M, x \models \psi$. Finally, \refitem{item_e_and_s}
    follows from \refitem{item_e_global} and \refitem{item_weakening} by taking
    $\psi = \phi$.
\end{proof}

Here \refitem{item_truths_sound} says that all truths are sound.
\refitem{item_e_global} says that expertise is global. \refitem{item_weakening} says
...
source does not have expertise) results in something true. \refitem{item_e_and_s}
...
\refitem{item_frame_condition_intersections} and \cref{prop_validities}
\refitem{item_e_and_s}, the language fragment $\cLSA$ containing only the $\S$
...
\begin{figure}[h]
    ...
    \caption{
        ...
        \cref{ex_economist_formalisation} (with reflexive edges omitted). Note
...
\begin{proof}\leavevmode
    \begin{enumerate}
        ...
        \item Take any reflexive and transitive relation $R$. Set
              ...
              %
              By \refitem{item_rp_ref_and_tr}, $R_P$ is reflexive and
              ...
              \refitem{item_rp_dc_property} and the definition of $P$ that
              \[
              \begin{aligned}
...
\begin{proof}
    ...
    \cref{lemma_p_to_rp_mapping} \refitem{item_rp_ref_and_tr} shows that $f$ is
    ...
    \refitem{item_rp_dc_property}, since $P$ is fully determined by $R_P$ for
    expertise models closed under unions and intersections. Finally,
    \cref{lemma_p_to_rp_mapping} \refitem{item_rp_surjectivity} shows that $f$
    is surjective.
\end{proof}
...
\begin{proof}
    ...
    \cref{lemma_p_to_rp_mapping} \refitem{item_rp_ref_and_tr}. We show $R_P$ is
    ...
    \refitem{item_rp_surjectivity}, that $f^{-1}(M^*) = (X, P, V)$, where $A
...
\begin{proof}
    ...
    \Mcompl$, \refeqn{eqn_s4s5_2} follows from \refeqn{eqn_s4s5_1}. We show
    \refeqn{eqn_s4s5_1} only.
    %
    Let $M = (X, P, V) \in \Mint \cap \Munions$. Write $f(M) =
    (X, R, V)$. From the definition of $f$ and
    \cref{lemma_p_to_rp_mapping} \refitem{item_rp_dc_property}, we have
    \[
        A \in P \iff A \text{ is downwards closed wrt R} \qquad(*)
    \]
    We show \refeqn{eqn_s4s5_1} by induction. The only non-trivial cases are
    $\E$ and $\S$ formulas.
...
\begin{proof}\leavevmode
    \begin{enumerate}
        ...
        \item By propositional logic we have $\thm ((\phi \limplies \psi) \land
              ...
              \refitem{item_mcs_mem_entail} we get $\psi \in \Gamma$.

        \item If $\neg\phi \in \Gamma$ then clearly $\phi \notin \Gamma$, since
              otherwise $\Gamma$ would be inconsistent. If $\phi \notin \Gamma$
              then $\Gamma \not\entails \phi$ by \refitem{item_mcs_mem_entail}.
        ...
        \item If $\phi \land \psi \in \Gamma$ then both $\Gamma \entails \phi$
              ...
              \refitem{item_mcs_mem_entail}.  Conversely, if $\phi, \psi \in
              \Gamma$ then $\Gamma \entails \phi \land \psi$, so $\phi \land
              \psi \in \Gamma$ by \refitem{item_mcs_mem_entail} again.
...
\begin{enumerate}
    ...
    \item Note that by \refitem{item_xsigma_mem} we have
        \[
        \begin{aligned}
...
\begin{proof}
    ...
    \cref{lemma_xsigma_properties} gives $|\phi|_\Gamma = X_\Gamma$. But since
    ...
    \cref{lemma_xsigma_properties}. The converse holds by symmetry.
...
\begin{proof}
    ...
    \refitem{item_frame_condition_intersections}.
...
\Cref{prop_shared_common_collapse} shows that when interpreting collective
...
\begin{proof}
    ...
    \refitem{item_rp_ref_and_tr}. $R'$ is transitive by its definition as a
...
\end{proposition}
\begin{proof}
    We prove only \refitem{item_common_exp_fp}; the others are straightforward.
...
Validity \refitem{item_common_exp_fp} comes from the
...
\Cref{thm_collective_s4s5_translation} can be used to adapt any sound and
...
\cref{sec_closure_properties}, closure conditions correspond to assumptions
...
requirement, since we argued in section \cref{sec_closure_properties} that
...
\begin{proposition}
\label{prop_expinc_reduction_axioms}
...
\label{ex_dynamic}
    ...
    The report of the economist in \cref{ex_economist_motivation} can be
    modelled as $[\sndann(p \land \neg d)]$. Note that, with $M$ as in
    \cref{ex_economist_formalisation}, $\|\S(p \land \neg d)\|_M = \|p\|_M$.
    The updated model $M^{\sndann(p \land \neg d)}$ therefore consists only of
    the bottom half of $M$ as shown in \cref{fig_economist_example}. We see
...
\begin{proposition}
\label{prop_sndann_reduction_axioms}
The following formulas are valid on $\M$:
\[
\begin{aligned}
...
\begin{proposition}
\label{prop_dynamic_validities}
For any $\alpha, \beta \in \cLzero$, the following formulas are valid
on $\Mint$:

\begin{enumerate}
    \item\label{item_exp_sndann} $\E\alpha \liff \A[\sndann\alpha]\alpha$

    \item\label{item_weakening_sndann} $\A(\alpha \limplies \beta) \limplies
          [\expinc\beta][\sndann\alpha]\beta$

  \item\label{item_weakening_sndann_eq} $[\expinc\alpha][\sndann\alpha]\alpha$
...
In words, \refitem{item_exp_sndann} says that expertise on a propositional
formula $\alpha$ is equivalent to the guarantee that $\alpha$ is true whenever
it is soundly announced. \refitem{item_weakening_sndann} is essentially a
reformulation of \cref{prop_validities} \refitem{item_weakening}; it says that
...
stronger formula $\alpha$.  \refitem{item_weakening_sndann_eq} is the special
case of \refitem{item_weakening_sndann} with $\beta = \alpha$, which says that
