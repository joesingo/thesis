...
\textbf{Chapter outline.} In \cref{exp_sec_expertise_and_soundness} we give a
...
\begin{proof}
    ...
    Let $M = (X, P, V)$ be a model and $x \in X$. \refitem{exp_item_truths_sound} and
    \refitem{exp_item_e_global} are clear. For \refitem{exp_item_weakening}, suppose $M, x
    ...
    \|\psi\|_M$, i.e. $M, x \models \psi$. Finally, \refitem{exp_item_e_and_s}
    follows from \refitem{exp_item_e_global} and \refitem{exp_item_weakening} by taking
    $\psi = \phi$.
\end{proof}

Here \refitem{exp_item_truths_sound} says that all truths are sound.
\refitem{exp_item_e_global} says that expertise is global. \refitem{exp_item_weakening} says
...
source does not have expertise) results in something true. \refitem{exp_item_e_and_s}
...
\refitem{exp_item_frame_condition_intersections} and \cref{exp_prop_validities}
\refitem{exp_item_e_and_s}, the language fragment $\cLSA$ containing only the $\S$
...
\begin{proof}\leavevmode
    \begin{enumerate}
        ...
        \item Take any reflexive and transitive relation $R$. Set
              ...
              %
              By \refitem{exp_item_rp_ref_and_tr}, $R_P$ is reflexive and
              ...
              \refitem{exp_item_rp_dc_property} and the definition of $P$ that
              \[
              \begin{aligned}
...
\begin{proof}
    ...
    \cref{exp_lemma_p_to_rp_mapping} \refitem{exp_item_rp_ref_and_tr} shows that $f$ is
    ...
    \refitem{exp_item_rp_dc_property}, since $P$ is fully determined by $R_P$ for
    expertise models closed under unions and intersections. Finally,
    \cref{exp_lemma_p_to_rp_mapping} \refitem{exp_item_rp_surjectivity} shows that $f$
    is surjective.
\end{proof}
...
\begin{proof}
    ...
    \cref{exp_lemma_p_to_rp_mapping} \refitem{exp_item_rp_ref_and_tr}. We show $R_P$ is
    ...
    \refitem{exp_item_rp_surjectivity}, that $f^{-1}(M^*) = (X, P, V)$, where $A
...
\begin{proof}
    ...
    \Mcompl$, \refeqn{exp_eqn_s4s5_2} follows from \refeqn{exp_eqn_s4s5_1}. We show
    \refeqn{exp_eqn_s4s5_1} only.
    %
    Let $M = (X, P, V) \in \Mint \cap \Munions$. Write $f(M) =
    (X, R, V)$. From the definition of $f$ and
    \cref{exp_lemma_p_to_rp_mapping} \refitem{exp_item_rp_dc_property}, we have
    \[
        A \in P \iff A \text{ is downwards closed wrt R} \qquad(*)
    \]
    We show \refeqn{exp_eqn_s4s5_1} by induction. The only non-trivial cases are
    $\E$ and $\S$ formulas.
...
\begin{proof}\leavevmode
    \begin{enumerate}
        ...
        \item By propositional logic we have $\thm ((\phi \limplies \psi) \land
              ...
              \refitem{exp_item_mcs_mem_entail} we get $\psi \in \Gamma$.

        \item If $\neg\phi \in \Gamma$ then clearly $\phi \notin \Gamma$, since
              otherwise $\Gamma$ would be inconsistent. If $\phi \notin \Gamma$
              then $\Gamma \not\entails \phi$ by \refitem{exp_item_mcs_mem_entail}.
        ...
        \item If $\phi \land \psi \in \Gamma$ then both $\Gamma \entails \phi$
              ...
              \refitem{exp_item_mcs_mem_entail}.  Conversely, if $\phi, \psi \in
              \Gamma$ then $\Gamma \entails \phi \land \psi$, so $\phi \land
              \psi \in \Gamma$ by \refitem{exp_item_mcs_mem_entail} again.
...
\begin{enumerate}
    ...
    \item Note that by \refitem{exp_item_xsigma_mem} we have
        \[
        \begin{aligned}
...
\begin{proof}
    ...
    \refitem{exp_item_frame_condition_intersections}.
...
\begin{proof}
    ...
    \refitem{exp_item_rp_ref_and_tr}. $R'$ is transitive by its definition as a
...
\end{proposition}
\begin{proof}
    We prove only \refitem{exp_item_common_exp_fp}; the others are straightforward.
...
Validity \refitem{exp_item_common_exp_fp} comes from the
...
In words, \refitem{exp_item_exp_sndann} says that expertise on a propositional
formula $\alpha$ is equivalent to the guarantee that $\alpha$ is true whenever
it is soundly announced. \refitem{exp_item_weakening_sndann} is essentially a
reformulation of \cref{exp_prop_validities} \refitem{exp_item_weakening}; it says that
...
stronger formula $\alpha$.  \refitem{exp_item_weakening_sndann_eq} is the special
case of \refitem{exp_item_weakening_sndann} with $\beta = \alpha$, which says that
