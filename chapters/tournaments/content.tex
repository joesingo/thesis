...
analysis~\cite{gonzalez2014paired} (where players represent products and
the beating relation expresses the preferences of a consumer), search engines
\cite{slutzki2006scoring}, sports~\cite{bozoki2016application} and
other domains.
...
\begin{example}
    ...
    preferable to reward sources for correctly values on difficult objects, or
...
Note that this related to but not the same as the ranking problem of truth
...
iteratively update source trust scores on the basis of current \emph{estimates}
of the true values for objects. One could therefore consider a bipartite
tournament at each stage of the iteration.
...
it correctly. The ranking of students then reflects their proficiency, and the
ranking of questions reflects their difficulty. This may be particularly useful
in the context automated grading of crowdsourced questions provided by students
themselves, which may vary in their difficulty (see for example the PeerWise
system \cite{denny_peerwise_2008}).
