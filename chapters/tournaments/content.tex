...
\paragraph{Chapter outline.} In \Cref{tourn_sec_preliminaries} we define the
...
In \Cref{tourn_result_phicardint_axioms} we saw which of the axioms from
...
call \axiomref{rank-removal}, is a technical axiom obtained via a related
property specific to interleaving operators. The second, called
\axiomref{argmax}, says that the maximum rank in $\ale_K^\phi$ should coincide
with that of $\ale_K^{\phicardint}$. Together with \axiomref{dual} and
\axiomref{chain-def}, these will characterise $\phicardint$.
...
Note that \axiomref{SMI} is a property of the selection functions $f$ and $g$.
In principle, it is possible that an interleaving operator $\phi$ admits two
pairs of selection functions $(f, g)$ and $(f', g')$ such that \axiomref{SMI}
...
\axiomref{SMI} either holds for $\phi$ or does not, independently of the choice
of selection functions $f$ and $g$. Nevertheless, to avoid circularity we will
say $\phi$ satisfies \axiomref{SMI} if \emph{there exist} $f, g$ with the
\axiomref{SMI} property such that $\phi = \intop{f, g}$.

First, $\phicardint$ does indeed satisfy \axiomref{SMI}.

\begin{proposition}
    \label{tourn_result_phicardint_smi}
    $\phicardint$ satisfies \axiomref{SMI}.
\end{proposition}

\begin{proof}
    Let $f$ and $g$ denote the selection functions for $\phicardint$. Let $A_i,
    B_i$ and $K^-$ be as in the statement of \axiomref{SMI}. Note that for any
    $a_p \in A_i$,
    \begin{align*}
        K(a_p) \cap B_i
    ...
    Hence $\phicardint$ satisfies \axiomref{SMI}.
\end{proof}

Note that in the statement of \axiomref{SMI}, $[m'] = A_{K^-}$ and $[n'] =
B_{K^-}$. Consequently, \axiomref{SMI} implies that the ranks of $A$ and $B$
...
two \axiomref{SMI} operators agreeing on maximal ranks for all $K$ must in fact
be equal.

\begin{proposition}
    \label{tourn_result_smi_maximal_ranks_equal}
    Let $\phi$ and $\phi'$ be interleaving operators satisfying \axiomref{SMI}.
...
\begin{proof}
    ...
    $K^-$ be the associated sub-matrix, as in the statement of \axiomref{SMI}.
    From property \labelcref{tourn_item_f_sel_1} from
    \Cref{tourn_def_selectionfunction} for $f$ and \axiomref{SMI}, we have
    \begin{align}
    ...
    \labelcref{tourn_eqn_smi_maximal_ranks_equal_f} we see $f(K, A_i, B_i) =
...
interleaving operators with \axiomref{SMI}, since we only need to consider the
maximal ranks of $A$ and $B$. In fact, given that $\phicardint$ satisfies
\axiomref{dual} we only need to consider the $A$ ranking. The following axiom
...
together with \axiomref{dual}.

\begin{axiom}[argmax]
    $\max(A, {\ale_K^\phi}) = \argmax_{a \in A}{|K(a)|}$.
\end{axiom}

Note that \axiomref{argmax} does \emph{not} require $\ale_K^\phi$ to reduce to
$\phicount$, since we only consider $a$ such that $|K(a)|$ is \emph{maximal}.

Now, since \axiomref{chain-def} characterises interleaving operators, to obtain
a characterisation of $\phicardint$ among \emph{all} operators it suffices to
find an alternative version of \axiomref{SMI} which can be applied to any
...
In what sense does \axiomref{rank-removal} capture \axiomref{SMI}? In the
following we show it is \emph{equivalent} to \axiomref{SMI}, when taken with
\axiomref{chain-def}. We need a preliminary lemma.
...
\begin{proposition}
    ...
    $\phi$ satisfies \axiomref{chain-def} and \axiomref{rank-removal} if and
    only if $\phi$ is an interleaving operator satisfying \axiomref{SMI}.
\end{proposition}

\begin{proof}
    For the `if" direction, suppose $\phi = \intop{f, g}$ is an interleaving
    operator satisfying \axiomref{SMI}. Then $\phi$ satisfies
    \axiomref{chain-def} by \Cref{tourn_result_chaindef_iff_interleaving}. We
    show \axiomref{rank-removal}. Let $\{a_1, \ldots, a_{m'}\}$, $\{b_1,
    \ldots, b_{n'}\}$ and $K^-$ be as in the statement of
    \axiomref{rank-removal}.
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    We prove \labelcref{tourn_eqn_smi_rr_induction} by induction on $i$. Take
    ...
    \labelcref{tourn_eqn_smi_rr_induction} for $i = 0$.

    For the inductive step, suppose \labelcref{tourn_eqn_smi_rr_induction}
    ...
    part of \labelcref{tourn_eqn_smi_rr_induction} holds for $i + 1$. For the
    second part, using property \labelcref{tourn_item_g_sel_3} from
    ...
    \labelcref{tourn_eqn_smi_rr_induction} holds. The case $B^-_i = \emptyset$
    ...
    \intop{f, g}$ satisfies \axiomref{SMI}, we get that for all $s \in [m'']$
    and $t \in [n'']$,
    \[
    ...
    of $K$ formed by $A_{i + 1}$ and $B_{i + 1}$. Applying \axiomref{SMI} in
    ...
    induction, this
    completes the proof of \labelcref{tourn_eqn_smi_rr_induction}.
    ...
    \begin{align*}
        ...
        &= -1 + r_K(a_p),
    \end{align*}
    where we use \labelcref{tourn_eqn_smi_rr_induction} and the fact that $a_p
    ...
    \axiomref{rank-removal}. The case for the $B$ ranking is identical, and we
    are done.

    Now for the `only if" direction, suppose $\phi$ satisfies
    \axiomref{chain-def} and \axiomref{rank-removal}. By
    ...
    \axiomref{SMI} holds. Let $K$, $A_i = \{a_1, \ldots, a_{m'}\}, B_i = \{b_1,
    \ldots, b_{n'}\}$ and $K^-$ be as in the statement of \axiomref{SMI}.
    ...
    \axiomref{rank-removal} $i - 1$ times, we get
    \[
        a_p \ale_K^\phi a_{p'}
        \iff p \ale_{K^-}^\phi p'
    \]
    ...
    \begin{align*}
        ...
        &\iff p \in f(K^-, [m'], [n'])
    \end{align*}
    as required for \axiomref{SMI}. One can show $b_q \in g(K, A_i, B_i) \iff q
...
\begin{theorem}
    $\phicardint$ is the unique operator satisfying \axiomref{dual},
    \axiomref{chain-def}, \axiomref{rank-removal} and \axiomref{argmax}.
\end{theorem}

\begin{proof}
    We have already seen in \Cref{tourn_result_phicardint_axioms} that
    $\phicardint$ satisfies \axiomref{dual} and \axiomref{chain-def}. For
    \axiomref{argmax}, note that for any tournament $K$,
    \begin{align*}
    ...
    $\phicardint$. Finally, \axiomref{rank-removal} follows from
    \Cref{tourn_result_rank_removal_smi} since $\phicardint$ is an interleaving
    operator with \axiomref{SMI} (by \Cref{tourn_result_phicardint_smi}).

    For uniqueness, suppose some operator $\phi$ also satisfies the stated
    axioms. By \Cref{tourn_result_rank_removal_smi}, $\phi$ is an interleaving
    operator satisfying \axiomref{SMI}. To show $\phi = \phicardint$ it is
    ...
    Clearly this is the case for the ranking of $A$, since \axiomref{argmax}
    completely prescribes $\max(A, {\ale_K^\phi})$ solely in terms of $K$.
    Moreover, by \axiomref{dual} we have ${\ble_K^\phi} =
    {\ale_{\dual{K}}^\phi}$, so
    \begin{align*}
        \max(B_K, {\ble_K^\phi})
    ...
    \end{align*}
    where we apply \axiomref{argmax} for $\phi$ and $\phicardint$ to the dual
