...
\begin{definition}%[Operator]
    ...
    A \emph{tournament ranking operator} $\T$ assigns each tournament $K$ a
...
\emph{strategy-proofness}: $a$ cannot improve its ranking by deliberately losing
...
\begin{figure*}
    ...
    \end{subfigure}
    \caption[
        Illustration of the proof of \cref{tourn_result_chainmin_mon_swapping}.
    ]{
        Depictions of the sets in \Cref{tourn_eqn_swap_lemma_symdiffs}.
    }
   ...
   \label{tourn_def_matchpref_operator}
    ...
    \begin{equation}
        ...
        = \argmin_{K' \in \minch{K}}{
            \vect_{\trianglelefteq}(K \ominus K')
        }
    \end{equation}
    where $[K \ominus K']_{ab} = |K_{ab} - K_{ab}'|$, and the minimum is taken
...
\footnotetext{
    Note that $K \ominus K'$ is 1 in exactly the entries where $K$ and $K'$
    differ.
}
...
\begin{example}
    ...
    \times \N$ so that $\vect_{\trianglelefteq}(K \ominus K')$ is obtained by
    collecting the entries of $K \ominus K'$ row-by-row, from top to
    ...
    $v_i = \vect_{\trianglelefteq}(K \ominus K_i)$, we have
    \begin{align*}
...
\begin{proof}
    ...
    notation, write $v = \vect_{\trianglelefteq}(K \ominus
    \alpha_{\trianglelefteq}(K))$ and $v' = \vect_{\trianglelefteq}(K \ominus K')$.
    There are two cases.
...
\begin{proof}
    ...
    \in A$. Since $N_1 \subset \cdots \subset N_s$, we have
    \begin{equation}
...
\begin{proof}
    ...
    holds for some $i \ge 0$. We consider three cases. First, suppose $A^-_i =
    ...
    \cref{tourn_eqn_smi_rr_induction} holds.

    The case $B^-_i = \emptyset$
...
\emph{Kemeny's rule} \cite{brandt2016a}, which find minimal sets of edges to
