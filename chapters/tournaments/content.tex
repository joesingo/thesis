...
\begin{example}
    ...
    \ldots, a_m\}$ provide possible answers to a number of questions. Among
    these questions, the `ground truth' answers are known for a subset $\{b_1,
    \ldots, b_n\}$, and thus for any pair $(a, b)$ it is known whether $a$
    provided the correct answers or not. A natural question arises: how can the
    sources be ranked based on their trustworthiness?

    The straightforward approach of simply counting the number of correct
    answers fails when the questions differ in their \emph{difficulty}. Indeed,
    it may be preferable to reward sources for correctly answering difficult
    questions, or to penalise them for failing easy questions. Furthermore, the
    notion of difficulty is not intrinsic to a question, but depends on how
    easily sources are able to answer it.\footnotemark{}

    The setting of bipartite tournament ranking addresses these issues.
    Indeed, the two kinds of `players' are the sources and questions; a source
    `beats' a question by answering it correctly and otherwise the question
    beats the source. While we wish to compare sources based on their
    trustworthiness and questions based on their difficulty, there are no
    direct source-to-source or question-to-questions comparisons available: the
    ranking must be constructed on the basis of the indirect patterns of
    correctness between the set of sources and questions.
\end{example}

\footnotetext{
    This is reminiscent of the mutual dependence between trustworthiness of
    sources and belief in claims in truth discovery in \cref{chapter_td}.
}

The truth discovery setting described in \cref{tourn_ex_td_example} clearly
applies in the case of \emph{semi-supervised truth
discovery}~\cite{yin_supervised_2011}, in which a subset of ground truth is
available. However, even when no such ground truth is available, many recursive
operators (including those described in \cref{td_new_sec_recursive_operators})
update source trust scores on the basis of the current \emph{estimate} of the
true claims; one could therefore consider a bipartite tournament at each stage
of the iteration.

A related example education~\cite{jiao2017algorithms}, where $A$ represents
students, $B$ exam questions, and student $a$ `beats' question $b$ by answering
it correctly. As in the truth discovery case, the ranking of students reflects
their proficiency, and the ranking of questions reflects their difficulty.
This may prove particularly useful in the context automated grading of
crowdsourced questions provided by students themselves, which may vary in their
difficulty (see for example the PeerWise system \cite{denny_peerwise_2008}).
...
and $B$ golf courses). In the remainder of this chapter we take the abstract
view in which $A$ and $B$ are simply `players', while keeping
\cref{tourn_ex_td_example} in mind as our motivating example.
...
\paragraph{Contributions.} Our primary contribution is the introduction of a
...
orthogonal components of a player's capabilities. \todo{expertise example;
topics.}
Nevertheless, if skill levels
...
domains. The question of how best to adapt chain editing in these cases remains
open.
